			...
			<article class="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
				...
				>
					...
					<aside class="space-y-4 bg-zinc-50/50 p-4">
						{#if canReconcile}
							<div>
								<h4 class="text-xs font-semibold text-zinc-500 uppercase">Resolve region</h4>
								<p class="mt-2 text-xs leading-5 text-zinc-600">
									The imported region is {candidate.region_key ?? 'missing'}; the OSM address
									resolves to {candidate.detected_region_key ?? 'an unknown region'}.
								</p>
								<form
									method="POST"
									action="?/reconcileRegion"
								...
								>
									...
									<label class="block">
										<span class="text-[11px] font-medium text-zinc-600">Correct region code</span>
										<select
											name="region_code"
											required
											value={candidate.detected_region_key ?? ''}
											class="mt-1 h-10 w-full rounded-md border-zinc-300 px-3 text-xs"
										>
											<option value="" disabled>Select a region</option>
											{#each data.regionCodes as region}<option value={region.code}>
													{region.code} · {region.region_name}
												</option>{/each}
										</select>
									</label>
									<label class="block">
										<span class="sr-only">Region reconciliation note</span>
										<input
											name="note"
									...
									<button
										class="h-9 w-full rounded-md bg-teal-700 px-3 text-xs font-semibold text-white hover:bg-teal-800"
										>Apply region and continue</button
									>
								</form>
							</div>
						{:else if canAct}
