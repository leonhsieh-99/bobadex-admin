...
<main class="mx-auto max-w-6xl space-y-8 px-5 py-7 sm:py-9">
	...
	{#if data.sourceErrors.length}
		<div
			class="flex items-start justify-between gap-4 border-l-4 border-amber-400 bg-amber-50 px-4 py-3 text-sm text-amber-900"
		>
			<div>
				<p class="font-medium">Some dashboard sources could not be loaded</p>
				<p class="mt-0.5 text-amber-800">
					{data.sourceErrors.join(', ')}. Other metrics are still current.
				</p>
			</div>
		</div>
	{/if}
	...
	<section aria-label="Key metrics" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
		<a
			href="/admin/reviews"
			class="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm transition hover:border-zinc-300 hover:shadow"
		>
			...
			<p class="mt-3 text-3xl font-semibold text-zinc-950">
				{number.format(data.metrics.reviewQueue)}
			</p>
			<p class="mt-1 text-xs text-zinc-500">
				{data.metrics.needsReview} storefronts need a tenant decision
			</p>
		</a>
		...
		>
			<div class="flex items-center justify-between gap-3">
				...
				<span
					class="h-2.5 w-2.5 rounded-full {data.metrics.failedJobs
						? 'bg-red-500'
						: data.metrics.activeJobs
							? 'bg-blue-500'
							: 'bg-emerald-500'}"
				></span>
			</div>
			<p class="mt-3 text-3xl font-semibold text-zinc-950">
				{number.format(data.metrics.activeJobs)}
			</p>
			<p class="mt-1 text-xs {data.metrics.failedJobs ? 'text-red-600' : 'text-zinc-500'}">
				{data.metrics.failedJobs
					? `${data.metrics.failedJobs} failed job needs attention`
					: 'No failed jobs'}
			</p>
		</a>
		...
		>
			...
			<p class="mt-3 text-3xl font-semibold text-zinc-950">
				{number.format(data.metrics.pendingIntake)}
			</p>
			<p class="mt-1 text-xs text-zinc-500">
				{data.metrics.pendingBrands} brands · {data.metrics.pendingReports} reports
			</p>
		</a>
		...
		<div class="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
			...
			<p class="mt-3 text-3xl font-semibold text-zinc-950">
				{number.format(data.metrics.brandCount)}
			</p>
		...
		<div class="mb-3 flex items-end justify-between gap-4">
			<div>
				...
				<p class="mt-1 text-sm text-zinc-500">
					Research queue health, publication coverage, and evidence quality.
				</p>
			</div>
			<a href="/admin/enrichment" class="text-sm font-medium text-zinc-700 hover:text-zinc-950"
				>Manage enrichment</a
			>
		</div>
		<div class="grid border-y border-zinc-200 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
			{#each [['Queue depth', data.metrics.enrichmentQueue, 'text-zinc-950'], ['Failed jobs', data.metrics.failedEnrichmentJobs, data.metrics.failedEnrichmentJobs ? 'text-red-700' : 'text-zinc-950'], ['Dossiers', data.metrics.dossiersNeedingReview, data.metrics.dossiersNeedingReview ? 'text-amber-700' : 'text-zinc-950'], ['Published', data.metrics.publishedProfiles, 'text-emerald-700'], ['Due refreshes', data.metrics.dueRefreshes, 'text-zinc-950'], ['Open flags', data.metrics.openIntegrityFlags, data.metrics.openIntegrityFlags ? 'text-red-700' : 'text-zinc-950']] as metric}
				<a
					href="/admin/enrichment"
					class="border-b border-zinc-200 px-4 py-5 last:border-b-0 hover:bg-zinc-50 sm:border-r xl:border-b-0 xl:last:border-r-0 sm:[&:nth-child(2n)]:border-r-0 lg:[&:nth-child(2n)]:border-r lg:[&:nth-child(3n)]:border-r-0 xl:[&:nth-child(3n)]:border-r"
				>
	...
	<section>
		<div class="mb-3">
			...
			<p class="mt-1 text-sm text-zinc-500">
				Work waiting on an admin decision or a failed pipeline step.
			</p>
		</div>
		<div class="divide-y divide-zinc-200 border-y border-zinc-200">
			<a
				href="/admin/reviews?tab=manual"
				class="flex items-center justify-between gap-4 px-1 py-3 hover:bg-zinc-50"
				><span class="text-sm text-zinc-700">Storefront dossiers</span><strong
					class="text-zinc-950 tabular-nums">{data.metrics.reviewQueue}</strong
				></a
			>
			<a
				href="/admin/enrichment"
				class="flex items-center justify-between gap-4 px-1 py-3 hover:bg-zinc-50"
				><span class="text-sm text-zinc-700">Brand dossiers</span><strong
					class="text-zinc-950 tabular-nums">{data.metrics.dossiersNeedingReview}</strong
				></a
			>
			<a
				href="/admin/imports"
				class="flex items-center justify-between gap-4 px-1 py-3 hover:bg-zinc-50"
				><span class="text-sm text-zinc-700">Failed pipeline jobs</span><strong
					class="tabular-nums {data.metrics.failedJobs + data.metrics.failedEnrichmentJobs
						? 'text-red-700'
						: 'text-zinc-950'}"
					>{data.metrics.failedJobs + data.metrics.failedEnrichmentJobs}</strong
				></a
			>
			<a
				href="/admin/brands"
				class="flex items-center justify-between gap-4 px-1 py-3 hover:bg-zinc-50"
				><span class="text-sm text-zinc-700">Brand submissions and reports</span><strong
					class="text-zinc-950 tabular-nums">{data.metrics.pendingIntake}</strong
				></a
			>
			<a
				href="/admin/imports#automation"
				class="flex items-center justify-between gap-4 px-1 py-3 hover:bg-zinc-50"
				><span class="text-sm text-zinc-700">Missing worker schedules</span
				>{#if data.metrics.missingCrons == null}<span class="text-xs font-medium text-zinc-500"
						>Status unavailable</span
					>{:else}<strong
						class="tabular-nums {data.metrics.missingCrons ? 'text-amber-700' : 'text-zinc-950'}"
						>{data.metrics.missingCrons}</strong
					>{/if}</a
			>
		</div>
	</section>
	...
	<section>
		<div class="mb-4 flex items-end justify-between">
			...
			<span class="text-sm text-zinc-500 tabular-nums"
				>{number.format(data.metrics.totalCandidates)} total candidates</span
			>
		</div>
		...
		<div class="grid border-y border-zinc-200 lg:grid-cols-3 lg:divide-x lg:divide-y-0">
			<div class="border-b border-zinc-200 py-5 lg:border-b-0 lg:pr-6">
				<div class="flex items-center justify-between">
					...
					<span
						class="rounded px-2 py-1 text-xs font-medium {statusClasses(
							data.latestImport?.status ?? 'idle'
						)}">{data.latestImport?.status ?? 'No run'}</span
					>
				</div>
				<p class="mt-3 text-2xl font-semibold text-zinc-950">
					{data.latestImport?.scope_label ?? 'No region'}
				</p>
				{#if data.latestImport}
					...
					<p class="mt-2 text-xs text-zinc-500">
						{data.latestImport.completedShards} of {data.latestImport.shardTotal} shards · started {relativeDate(
							data.latestImport.started_at ?? data.latestImport.created_at
						)}
					</p>
				{:else}
				...
				<div class="mt-4 grid grid-cols-3 gap-2 text-xs">
					<div>
						<p class="font-semibold text-zinc-900">
							{data.pipeline.candidateStatusCounts.ready_for_enrichment ?? 0}
						</p>
						<p class="text-zinc-500">ready</p>
					</div>
					<div>
						<p class="font-semibold text-zinc-900">{data.metrics.reviewQueue}</p>
						<p class="text-zinc-500">exceptions</p>
					</div>
					<div>
						<p class="font-semibold text-red-700">
							{data.pipeline.candidateStatusCounts.known_negative ?? 0}
						</p>
						<p class="text-zinc-500">negatives</p>
					</div>
				</div>
			</div>
			...
			<div class="py-5 lg:pl-6">
				<div class="flex items-center justify-between">
					...
					<a href="/admin/imports" class="text-xs font-medium text-zinc-600 hover:text-zinc-950"
						>Manage</a
					>
				</div>
				<p class="mt-3 text-2xl font-semibold text-zinc-950">
					{number.format(data.pipeline.shardCounts.succeeded ?? 0)} succeeded
				</p>
				<div class="mt-4 grid grid-cols-3 gap-2 text-xs">
					<div>
						<p class="font-semibold text-zinc-900">{data.pipeline.shardCounts.queued ?? 0}</p>
						<p class="text-zinc-500">queued</p>
					</div>
					<div>
						<p class="font-semibold text-blue-700">{data.pipeline.shardCounts.processing ?? 0}</p>
						<p class="text-zinc-500">processing</p>
					</div>
					<div>
						<p class="font-semibold text-red-700">{data.pipeline.shardCounts.failed ?? 0}</p>
	...
	<section class="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
		<div>
			<div class="mb-3 flex items-center justify-between">
				<div>
					...
					<p class="mt-1 text-sm text-zinc-500">
						Storefront dossiers waiting for an identity decision.
					</p>
				</div>
				<a href="/admin/reviews" class="text-sm font-medium text-zinc-700 hover:text-zinc-950"
					>View all</a
				>
			</div>
			<div class="overflow-hidden rounded-lg border border-zinc-200 bg-white">
				{#each data.reviewCandidates as candidate}
					<a
						href={`/admin/reviews?tab=manual&q=${encodeURIComponent(candidate.canonical_name ?? '')}`}
						class="flex items-center justify-between gap-4 border-b border-zinc-100 px-4 py-3 last:border-b-0 hover:bg-zinc-50"
					>
						<div class="min-w-0">
							<p class="truncate text-sm font-medium text-zinc-900">
								{candidate.canonical_name ?? 'Unnamed candidate'}
							</p>
							<p class="mt-0.5 text-xs text-zinc-500">
								{candidate.region_key ?? 'Unknown region'} · {relativeDate(candidate.updated_at)}
							</p>
						</div>
						<span
							class="rounded px-2 py-1 text-xs font-medium {statusClasses(
								candidate.process_status
							)}">{candidate.process_status.replaceAll('_', ' ')}</span
						>
					</a>
				{/each}
				{#if data.reviewCandidates.length === 0}
					<div class="px-4 py-10 text-center text-sm text-zinc-500">
						The storefront queue is clear.
					</div>
				{/if}
			</div>
		</div>
		...
		<div>
			<div class="mb-3 flex items-center justify-between">
				...
				<a href="/admin/imports" class="text-sm font-medium text-zinc-700 hover:text-zinc-950"
					>Details</a
				>
			</div>
			<div class="overflow-hidden rounded-lg border border-zinc-200 bg-white">
				{#each data.recentImports as job}
					<div
						class="flex items-center justify-between gap-3 border-b border-zinc-100 px-4 py-3 last:border-b-0"
					>
						<div>
	...
	<section>
		...
		<div class="grid gap-4 sm:grid-cols-2">
			<a
				href="/admin/brands"
				class="rounded-lg border border-zinc-200 bg-white p-4 hover:border-zinc-300"
			>
				<div class="flex items-center justify-between">
				...
				<p class="mt-3 truncate text-sm text-zinc-500">
					{data.stagingRows[0]?.suggested_name ?? 'No pending submissions'}
				</p>
			</a>
			<a
				href="/admin/reports"
			...
			>
				...
				<p class="mt-3 truncate text-sm text-zinc-500">
					{data.reportRows[0]
						? `${data.reportRows[0].content_type}: ${data.reportRows[0].reason ?? 'No reason supplied'}`
						: 'No pending reports'}
				</p>
			</a>
		</div>
